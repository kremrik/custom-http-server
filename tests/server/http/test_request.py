...
class test_LazyRequest(unittest.IsolatedAsyncioTestCase):
    async def test_start_line(self):
        ...
        with self.subTest("method"):
            self.assertEqual(request.Method.GET, await lazy_request.method)
        with self.subTest("path"):
            self.assertEqual("/path/to/resource", await lazy_request.path)
        with self.subTest("protocol"):
            self.assertEqual(request.Protocol.HTTP1_1, await lazy_request.protocol)
